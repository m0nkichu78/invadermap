...
import { getInvaderById } from "@/lib/data/invaders";
...
export default function CollectionPage() {
  ...
  const collection = Object.entries(scans)
    .map(([invaderId, scanStatus]) => ({
      invader: getInvaderById(invaderId),
      scanStatus,
    }))
    .filter(
      (item): item is { invader: NonNullable<typeof item.invader>; scanStatus: ScanStatus } =>
        item.invader !== undefined
    );
    ...
    <div className="min-h-dvh gradient-mesh pb-20">
      ...
      <div className="px-3 pt-4">
        {filtered.length === 0 ? (
          ...
          >
            {filtered.map(({ invader, scanStatus }) => {
              const Icon = SCAN_ICON[scanStatus];
              return (
                <motion.div key={invader.id} variants={cardVariants}>
                  <Link href={`/invader/${invader.id}`}>
                    <div className={`bg-[--surface] border border-[--border] border-l-4 ${SCAN_LEFT_BORDER[scanStatus]} rounded-lg p-2.5 flex flex-col gap-1 active:scale-[0.97] transition-transform duration-150`}>
                      ...
                      <p className="text-[10px] text-[--text-muted] uppercase tracking-wider truncate">{invader.city}</p>
